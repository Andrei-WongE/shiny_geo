...
server <- function(input, output, session) {
  ...
  output$dynamicPlot <- renderUI({
    if (input$sidebarMenu == "eFUA") {
      if (input$section == "GDP per capita (PPP)") {
        ...
        plotOutput("gvaCompositionPlot_eFUA", height = "600px"
                   # , width = "100%"
                   )
        
      } else if (input$section == "Composition of Employment") {
        plotOutput("empCompositionPlot_eFUA", height = "600px"
                   # , width = "100%"
                   )
      }
    }
  ...
  output$gvaCompositionPlot_eFUA <- renderPlot({
    if (input$sidebarMenu == "eFUA" && input$section == "Composition of GVA") {
      ...
      # Function to create GVA composition plot
      create_gva_plot <- function(data, title, color) {
        if (nrow(data) > 0) {
          plot_data <- data %>%
            pivot_longer(
              cols = all_of(columns_to_pivot_GVA),
              names_to = "GVA_sector",
              values_to = "Percentage"
            ) %>%
            mutate(GVA_sector = gsub("(_GVA_Pct|_)", " ", GVA_sector))
          
          ggplot(plot_data, aes(x = Location, y = Percentage, fill = GVA_sector)) +
            geom_bar(stat = "identity", position = "fill", alpha = 0.8) +
            scale_fill_manual(values = RColorBrewer::brewer.pal(min(length(unique(plot_data$GVA_sector)), 11), "Set3")) +
            labs(title = title, x = NULL, y = "Percentage", fill = "GVA Sector") +
            theme_minimal() +
            scale_y_continuous(labels = scales::percent_format()) +
            theme(
              axis.text.x = element_text(angle = 0, hjust = 1, vjust = 0.5, size = 11
      ...
      if (!is.null(input$location) && input$location != "") {
        main_data <- filteredDataByYear_eFUA()
        if (nrow(main_data) > 0) {
          plot_list[[length(plot_list) + 1]] <- create_gva_plot(main_data, 
                                              paste("Main City:", input$location),
                                              "#cd402a")
        }
      }
      ...
      if (!is.null(input$comparators_eFUA) && length(input$comparators_eFUA) > 0) {
        comparison_data <- comparisonGroupDataByYear_eFUA()
        if (nrow(comparison_data) > 0) {
          plot_list[[length(plot_list) + 1]] <- create_gva_plot(comparison_data,
                                                "Comparison Cities",
                                                "#80E68A")
        }
      }
      ...
      if (!is.null(input$aspirational_eFUA) && length(input$aspirational_eFUA) > 0) {
        aspirational_data <- aspirationalGroupDataByYear_eFUA()
        if (nrow(aspirational_data) > 0) {
          plot_list[[length(plot_list) + 1]] <- create_gva_plot(aspirational_data,
                                                                 "Aspirational Cities",
                                                                 "#5BC8F0")
        }
      }
  ...
  output$empCompositionPlot_eFUA <- renderPlot({
    if (input$sidebarMenu == "eFUA" && input$section == "Composition of Employment") {
      ...
      # Function to create employment composition plot (stacked bars)
      create_emp_composition_plot <- function(data, title, color) {
        if (nrow(data) > 0) {
          plot_data <- data %>%
            pivot_longer(
              cols = all_of(columns_to_pivot_Emp),
              names_to = "Employment_sector",
              values_to = "Percentage"
            ) %>%
            mutate(Employment_sector = gsub("(_Emp_Pct|_)", " ", Employment_sector))
          ...
          ggplot(plot_data, aes(x = Location, y = Percentage, fill = Employment_sector)) +
            ...
            scale_fill_manual(values = RColorBrewer::brewer.pal(min(length(unique(plot_data$Employment_sector)), 11), "Spectral")) +
            labs(title = title, x = NULL, y = "Percentage", fill = "Employment Sector") +
            theme_minimal() +
            scale_y_continuous(labels = scales::percent_format()) +
            theme(
              axis.text.x = element_text(angle = 0, hjust = 1, vjust = 0.5, size = 11
      ...
      if (!is.null(input$location) && input$location != "") {
        main_data <- filteredDataByYear_eFUA()
        if (nrow(main_data) > 0) {
          plot_list[[length(plot_list) + 1]] <- create_emp_composition_plot(main_data, 
                                              paste("Main City:", input$location),
                                              "#cd402a")
        }
      }
      ...
      if (!is.null(input$comparators_eFUA) && length(input$comparators_eFUA) > 0) {
        comparison_data <- comparisonGroupDataByYear_eFUA()
        if (nrow(comparison_data) > 0) {
          plot_list[[length(plot_list) + 1]] <- create_emp_composition_plot(comparison_data,
                                                "Comparison Cities",
                                                "#80E68A")
        }
      }
      ...
      if (!is.null(input$aspirational_eFUA) && length(input$aspirational_eFUA) > 0) {
        aspirational_data <- aspirationalGroupDataByYear_eFUA()
        if (nrow(aspirational_data) > 0) {
          plot_list[[length(plot_list) + 1]] <- create_emp_composition_plot(aspirational_data,
                                                                 "Aspirational Cities",
                                                                 "#5BC8F0")
        }
      }

...
server <- function(input, output, session) {
  ...
  similarData <- reactive({
    ...
    employment_cols <- c("Public_Services_Emp_Pct", "Industry_Emp_Pct", 
  ...
  urbanDatacomparators <- reactive({
      req(input$comparators)
      built_ucdb %>%
          filter(Urban_centre %in% input$comparators)
  })
  ...
  output$urbanPlotCity <- renderPlot({
      ...
      ggplot(combined_data, aes(x = reorder(Urban_centre, Built_rel_change), y = Built_rel_change, fill = Urban_centre == input$urbanCentre)) +
          geom_bar(stat = "identity") +
          scale_fill_manual(values = c("TRUE" = "red", "FALSE" = "gray80")) +
          labs(title = paste("Built-up Surface Relative Change for", input$urbanCentre, "and Comparison Group"),
  ...
  output$urbanPlotcomparators <- renderPlot({
      ...
      ggplot(combined_data, aes(x = reorder(Urban_centre, Built_rel_change), y = Built_rel_change, fill = Urban_centre == input$urbanCentre)) +
          geom_bar(stat = "identity") +
          scale_fill_manual(values = c("TRUE" = "red", "FALSE" = "gray80")) +
          labs(title = paste("Built-up Surface Relative Change in", input$comparators, "with", input$urbanCentre),
              x = "Urban Centre", y = "Built-up Surface Relative Change") +
          theme_minimal() +
          theme(legend.position = "none")
  })

...
server <- function(input, output, session) {
  ...
  output$dynamicPlot <- renderUI({
    if (input$sidebarMenu == "eFUA") {
      if (input$section == "GDP per capita (PPP)") {
        ...
        plotOutput("employmentPlot_eFUA")
      }
    }
  })
  ...
  aspirationalGroupData_eUFA <- reactive({
  req(input$aspirational_eFUA)
  oe_data_shi %>%
    filter(Location %in% input$aspirational_eFUA)
  })

aspirationalGroupDataByYear_eUFA <- reactive({
  ...
  output$dataTable <- DT::renderDataTable({
    if (input$sidebarMenu == "eFUA") {
        ...
        aspirational_data <- aspirationalGroupDataByYear_eUFA() %>%
          select(-ends_with("Emp_Pct"))
   ...
   output$plotOutput_eFUA <- renderPlot({
    if (input$sidebarMenu == "eFUA" && input$section == "GDP per capita (PPP)") {
      ggplot() +
        ...
        geom_line(
          data = aspirationalGroupData_eUFA(),
          aes(x = Year, y = GDP_per_capita_PPP, group = Location),
          color = "#5BC8F0",
          alpha = 0.6
        ) +
        ...
        geom_text_repel(
          data = aspirationalGroupData_eUFA() %>% filter(Year == max(oe_data_shi$Year)),
          aes(x = Year, y = GDP_per_capita_PPP, label = Location),
          color = "#5BC8F0",
          size = 5,
          direction = "y",
  ...
  output$employmentPlot_eFUA <- renderPlot({
    if (input$sidebarMenu == "eFUA" && input$section == "Employment structure") {
      ...
      selected_city_data <- filteredDataByYear_eFUA()
      comparator_data <- comparisonGroupDataByYear_eFUA()
      ...
      combined_data <- selected_city_data %>%
       ...
       filter(Location == input$location | Location %in% input$comparators_eUFA | Location %in% input$aspirational_eFUA) %>%
        pivot_longer(
